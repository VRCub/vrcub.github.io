...
青春版是为了给不满足推荐配置"标准版"的整合版本，你可以选择他，但是体验可能会有所欠缺(目前是一个陈旧的版本，必要情况下使用)
:::
...
<Tabs groupId="launcher" className="unique-tabs" queryString>
  <TabItem value="pcl2" label="使用 PCL2" default>
    PCL2作为一个特别老牌的启动器，其稳定性与可靠性还有快速上手性都非常的出色，但因为其不支持跨端导致其用户大部份为Windows用户（虽说只有极少数人用MacOS来玩和Linux来玩罢了）

    在浏览器地址栏输入 afdian.com/p/0164034c016c11ebafcb52540025c377 并访问龙腾猫跃在爱发电发布的下载地址

    推荐使用下载2/下载3

    下载即可
  </TabItem>
  <TabItem value="launcherx" label="使用 LancherX">
    在浏览器地址栏输入 corona.studio/lx 并访问

    点击立即下载，并选择合适的系统版本

    下载后解压即可
  </TabItem>
  <TabItem value="hmcl" label="使用 HMCL">
    HMCL非常强悍且简洁，并且有非常多的功能（模组/世界/资源包/整合包等下载，目前不支持光影下载；更简单的模组管理，模组相关跳转；原理图管理；更简单的世界管理；Java自定义下载 等一系列功能）

    在浏览器地址栏输入 hmcl.huangyuhui.net/download/ 并访问

    选择下载方式,此处推荐使用开发版，下载速度更快,兼容Windows/Linux/Mac OS

    cn.bing.com搜索官网下载即可
  </TabItem>
  <TabItem value="modrinth" label="使用 Modrinth APP">
    Modrinth App是一个新晋的启动器兼模组下载平台，收录50000+的模组/资源包/数据包/光影/整合包/插件。

    但缺点很明显"没有中文支持"，但是如果你能看得懂的话，会发现这也挺好用的

    在浏览器地址栏输入 modrinth.com/app 并访问

    点击"下载Modrinth App"，或者选择"更多下载方式"
...
Modrinth App请找到设置（通常位于左下角）→资源管理→应用目录，**更改为C盘以外的驱动盘文件夹**

:::info
 这一步是为了防止C盘占用与方便后期操作
:::

下面有指导方式，但其实大差不差，请选择你的启动器再来按照流程来进行安装

:::note[请选择一个启动器来查看教程]

<Tabs groupId="launcher" className="unique-tabs" queryString>
  <TabItem value="pcl2" label="使用 PCL2" default>
    双击PCL2图标，打开启动器，同意条款，请将你在第一章的文件文件拖动到PCL2的窗口上，并根据操作完成安装
  </TabItem>
  <TabItem value="launcherx" label="使用 LancherX">
    打开启动器，一般按照指引一步步操作即可
    ...
    激活LancherX后，请将你在第一章的文件拖动到LancherX的窗口上，并根据操作完成安装
  </TabItem>
  <TabItem value="hmcl" label="使用 HMCL">
    打开启动器，下载Java，同意条款，将你在第一章的文件解压,定位至VRCub_ver，并找到"modpack.mrpack",拖入至启动器，并且"安装"，等待整合包安装完成
  </TabItem>
  <TabItem value="modrinth" label="使用 Modrinth APP">
    请打开启动器，找到设置（通常位于左下角）→Java下载,并且下载Java21至你要下载到的文件夹，登录你的正版账号，

    将你在第一章的文件解压,定位至VRCub_ver，并找到"modpack.mrpack"，直接打开
    <Admonition type="warning" title="注意！">
      默认情况，若非默认在左方"+"安装时选择"从文件安装"，并且将文件导入即可，等待整合包安装完成
    </Admonition>
  </TabItem>
</Tabs>
...
确认好你已经登录你的正版账号，启动你的游戏即可！

如果你是LancherX，可以将VRCub开头的版本旁的图钉图标点上，方便使用此整合包进入游戏

---

## 常见问题解答

到这里，新手教程就结束了，下面呢还准备了一些应对方案供玩家查阅：
